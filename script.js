...
/* ==========================================================================
   RESUMO DO HISTORICO DA INJECAO POR PERIODO
   ========================================================================== */
function atlasInjecaoNumero(valor) {
    const numero = parseFloat(String(valor ?? '').replace(',', '.'));
    return Number.isFinite(numero) ? numero : 0;
}

function atlasInjecaoDataMs(dataPt) {
...
function atlasInjecaoDataIsoParaMs(dataIso, fimDia = false) {
    if (!dataIso) return 0;
...
function atlasInjecaoFormatarKg(valor) {
    return `${atlasInjecaoNumero(valor).toFixed(2)} kg`;
}

function atlasInjecaoFormatarM(valor) {
    return `${atlasInjecaoNumero(valor).toFixed(2)} m`;
}

function atlasInjecaoTodosRelatorios(modulo = 'injecao') {
    const db = JSON.parse(localStorage.getItem('atlas_db') || '{}');
    const relatorios = [];
        ...
        Object.keys(db[ano] || {}).forEach(mes => {
            (Array.isArray(db[ano][mes]) ? db[ano][mes] : []).forEach((rel, index) => {
                if (rel && rel.modulo === modulo) {
                    relatorios.push({ ...rel, ano, mes, index, dataMs: atlasInjecaoDataMs(rel.data) });
                }
            });
        });
    });
...
function atlasInjecaoResumo(relatorios) {
    const resumo = {
        relatorios: 0,
        itens: 0,
        metrosManha: 0,
    ...
    (Array.isArray(relatorios) ? relatorios : []).forEach(rel => {
        ...
        (Array.isArray(rel.itens) ? rel.itens : []).forEach(item => {
            const manha = atlasInjecaoNumero(item.metrosManha);
            const tarde = atlasInjecaoNumero(item.metrosTarde);
            resumo.itens += 1;
            resumo.metrosManha += manha;
            resumo.metrosTarde += tarde;
            resumo.metrosTotal += atlasInjecaoNumero(item.metros || manha + tarde);
            Object.keys(resumo.quimicos).forEach(chave => {
                resumo.quimicos[chave] += atlasInjecaoNumero(item[chave]);
            });
        });
    });
...
function atlasInjecaoResumoHTML(titulo, relatorios) {
    const resumo = atlasInjecaoResumo(relatorios);
    const q = resumo.quimicos;
    return `
        <div class="atlas-resumo-injecao" style="background:#0f172a; border:1px solid #334155; border-radius:10px; padding:14px; margin:10px 0 14px 0; color:white;">
            <div style="display:flex; justify-content:space-between; gap:10px; flex-wrap:wrap; border-bottom:1px solid #334155; padding-bottom:10px; margin-bottom:10px;">
                <div>
                    ...
                    <div style="color:#94a3b8; font-size:12px;">${resumo.relatorios} relatorio(s) | ${resumo.itens} item(ns)</div>
                </div>
                <div style="font-weight:900; color:#22c55e;">Total: ${atlasInjecaoFormatarM(resumo.metrosTotal)}</div>
            </div>
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(160px, 1fr)); gap:8px; margin-bottom:10px;">
                <div style="background:#020617; border:1px solid #1e293b; border-radius:8px; padding:10px;"><small style="color:#94a3b8;">Turno da manha</small><br><b>${atlasInjecaoFormatarM(resumo.metrosManha)}</b></div>
                <div style="background:#020617; border:1px solid #1e293b; border-radius:8px; padding:10px;"><small style="color:#94a3b8;">Turno da tarde</small><br><b>${atlasInjecaoFormatarM(resumo.metrosTarde)}</b></div>
                <div style="background:#020617; border:1px solid #1e293b; border-radius:8px; padding:10px;"><small style="color:#94a3b8;">Periodo inteiro</small><br><b>${atlasInjecaoFormatarM(resumo.metrosTotal)}</b></div>
            </div>
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(120px, 1fr)); gap:8px;">
                <div><small>POL</small><br><b>${atlasInjecaoFormatarKg(q.pol)}</b></div>
                <div><small>POL/PIR</small><br><b>${atlasInjecaoFormatarKg(q.polpir)}</b></div>
                <div><small>MDI</small><br><b>${atlasInjecaoFormatarKg(q.mdi)}</b></div>
                <div><small>PEN</small><br><b>${atlasInjecaoFormatarKg(q.pen)}</b></div>
                <div><small>CAT 1</small><br><b>${atlasInjecaoFormatarKg(q.cat1)}</b></div>
                <div><small>CAT 2</small><br><b>${atlasInjecaoFormatarKg(q.cat2)}</b></div>
                <div><small>CAT 3</small><br><b>${atlasInjecaoFormatarKg(q.cat3)}</b></div>
                <div><small>CAT 4</small><br><b>${atlasInjecaoFormatarKg(q.cat4)}</b></div>
            </div>
        </div>
    `;
}

function atlasInjecaoPeriodoSelecionado() {
    const tipo = document.getElementById('atlas-inj-resumo-periodo')?.value || 'mensal';
    const todos = atlasInjecaoTodosRelatorios('injecao');
    ...
    if (tipo === 'personalizado') {
        inicioMs = atlasInjecaoDataIsoParaMs(document.getElementById('atlas-inj-resumo-inicio')?.value, false) || 0;
        fimMs = atlasInjecaoDataIsoParaMs(document.getElementById('atlas-inj-resumo-fim')?.value, true) || Number.MAX_SAFE_INTEGER;
    }

    return todos.filter(rel => rel.dataMs >= inicioMs && rel.dataMs <= fimMs);
}

function atlasInjecaoAtualizarResumoPeriodo() {
    ...
    destino.innerHTML = atlasInjecaoResumoHTML(labels[tipo] || 'Resumo', atlasInjecaoPeriodoSelecionado());
    if (typeof window.atlasLimparTextoTela === 'function') window.atlasLimparTextoTela();
}

function atlasInjecaoControlesResumoHTML() {
    return `
...
window.exibirHistoricoModulo = function(modulo) {
    ...
    let html = `
        <div style="padding:15px; color:white;">
            ...
            ${atlasInjecaoControlesResumoHTML()}
    `;
    ...
    Object.keys(db || {}).forEach(ano => {
        ...
        Object.keys(db[ano] || {}).forEach(mes => {
            ...
            html += `
                ...
                <div id="${mesId}" style="display:none; padding-left:10px; border-left:2px solid #3b82f6; margin-bottom:10px;">
                    ${atlasInjecaoResumoHTML(`Resumo de ${mes} / ${ano}`, filtrados)}
            `;
    ...
    html += `</div>`;
    render.innerHTML = html;
    atlasInjecaoAtualizarResumoPeriodo();
    if (typeof window.atlasLimparTextoTela === 'function') window.atlasLimparTextoTela();
};
